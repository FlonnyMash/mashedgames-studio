...
```
packages/templates/src/lucky-wheel/
```

Where `lucky-wheel` is the specific template directory you have been assigned (e.g. `catch-game`, `quiz-game`).
...
- NEVER use relative `../` paths that escape `packages/templates/src/lucky-wheel/`.
...
export const myGameManifest = {
  templateId:       'lucky-wheel',    // must match directory name
  version:          '1.0.0',
...
1. Are all file writes inside `packages/templates/src/lucky-wheel/`? If not, STOP.
